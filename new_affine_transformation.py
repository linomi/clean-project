...
def transformer(data,image_shape = (304,608,1),affine_shape =(6) , name='SpatialTransformer2dAffine'):
    ...
    affines = data[:,tf.math.reduce_prod(image_shape):]
    affines = tf.reshape(affines,shape = (-1,affine_shape))
    output = _transform(affines,inputs, out_size)
    return output
